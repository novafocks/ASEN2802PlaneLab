...
for i = 1 : length(const.c.unique_aoa)
    % Defining current angle of attack and pressures
    misc.curr_aoa       = const.c.unique_aoa(i);
...
figure();
plot(const.c.unique_aoa, coef.lift);
hold on;
...
figure();
plot(const.c.unique_aoa, coef.drag);
hold on;
...
function c_P = c_P_calc(aoa, const)
    ...
    coef.pressure_aoa = coef.pressure(const.c.unique_aoa == aoa, :);
